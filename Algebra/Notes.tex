...
\textbf{Remark:} The inverse of $h \in G$ in a \textbf{multiplicative notation} is denoted by $h^{-1}$. When we are working with an \textbf{additive notation} $(G, +)$, the neutral element is denoted by 0 and the inverse of $h \in G$ is denoted by $-h$. \underline{In either case}, for an element $a$, we will denote the inverse of $a$ to be $a^{-1}$. 
...
\begin{lemma}{}{}
    ...
    \[(g * h)^{-1} = h^{-1} * g^{-1}\]
    Here, $h^{-1}$ and $g^{-1}$ are inverses of $h$ and $g$, respectively. Also, $(g * h)^{-1}$ is the inverse of $g * h$.  
\end{lemma}
...
% 41:49 
% https://www.youtube.com/watch?v=2XX-4W-r-Jg
