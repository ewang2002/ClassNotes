...
\textbf{Remarks:} 
\begin{itemize}
    \item Remember that the above only works for 2 arguments. For arbitrary arguments, the same idea still holds, but you need to generalize it. In particular, the first argument might be at \code{[rsp + 16]}, the second argument might be at \code{[rsp + 24]}, and so on. The last argument, then, would be in \code{rax}. Thus, you'll need to move these arguments into the correct places before you can call the function.
...
\end{center}
Under this assumption where the callee manages the stack pointer, the first thing the function will do is \code{sub rsp} so that there's enough space for all local variables. That is, we should move \code{[rsp]} so that we end up with a memory diagram taht looks like 
\begin{center}
...
% rsp is a register that holds the memory address of the current top of the stack. It is called the stack pointer because it points to the top of the stack. This register is used to manipulate the stack by pushing or popping data onto or off of the stack.
% [rsp] is an addressing mode that refers to the memory location pointed to by the RSP register. This is the memory location at the current top of the stack.
