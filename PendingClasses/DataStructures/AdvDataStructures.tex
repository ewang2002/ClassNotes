...
Consider the following AVL tree, where each entry is formatted like $(\text{Element}, \text{Balance Factor})$: 
\begin{verbatim}
          (5, 1)
            /\ 
           /  \ 
...
\begin{enumerate}[(1)]
    ...
    \end{itemize}
    We perform a left AVL rotation because \code{5} is the bad node and \code{10} is the heavier node; so, we can drop the heavier node in as to fix the priority system. % TODO needs to be more clear? 
